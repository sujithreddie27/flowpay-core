...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transactions_sender_status_created
    ON transactions (sender_id, status, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transactions_receiver_status_created
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transactions_sender_amount_date
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transactions_idempotency_key_status
    ON transactions (idempotency_key, status)
    WHERE idempotency_key IS NOT NULL;

-- Accounts: Index for active accounts balance lookups
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_accounts_user_currency_active
    ON accounts (user_id, currency)
    WHERE status = 'ACTIVE';

-- Partial index for retryable failed transactions
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transactions_retryable
    ON transactions (retry_count, updated_at)
    WHERE status = 'FAILED' AND retry_count < 3;

-- Index for stale pending transaction cleanup
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transactions_stale_pending
    ON transactions (updated_at)
    WHERE status = 'PENDING';

-- BRIN index for time-series audit log queries (very space efficient)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_audit_logs_created_at_brin
